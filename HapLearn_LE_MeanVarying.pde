...
void setup() {
  ...
  persinc = inctotal / table.getRowCount();
...
void draw() {
  ...
  if (plot.isOverBox(mouseX, mouseY)) {
    ...
    if (plot.getPointAt(mouseX, mouseY)!=null) 
      ...
      incX = plot.getPointAt(mouseX, mouseY).getX();
      if (Math.abs(persinc-incX) < 17000)
      {     
            if (Math.abs(persinc-incX)!= mInten)
            {
              mInten = Math.abs(persinc-incX);        //Absolute distance between cursor point and mean
              barH = map(mInten, 17000, 100, 0, 255); //Mapping distance to intensity
              ...
              rest_ms = 0;     // The NO vibration time or resting in ms before starting next cycle
